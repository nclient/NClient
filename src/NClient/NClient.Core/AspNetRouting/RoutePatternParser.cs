    ...
    internal static class RoutePatternParser
    {
        ...
        public static RoutePattern Parse(string pattern)
        {
            ...
            while (context.MoveNext())
            {
                ...
                if (context.Current == Separator)
                {
                    ...
                    throw new RoutePatternException(pattern, "The route template separator character '/' cannot appear consecutively. It must be separated by either a parameter or a literal value.");
                }
        ...
        private static bool IsAllValid(Context context, List<RoutePatternPathSegment> segments)
        {
            ...
            for (var i = 0; i < segments.Count; i++)
            {
                ...
                for (var j = 0; j < segment.Parts.Count; j++)
                {
                    ...
                    {
                        context.Error = "A catch-all parameter can only appear as the last segment of the route template.";
                        return false;
                    }
                }
            }
        ...
        private static bool IsSegmentValid(Context context, List<RoutePatternPart> parts)
        {
            ...
            for (var i = 0; i < parts.Count; i++)
            {
                ...
                {
                    context.Error = "A path segment that contains more than one section, such as a literal section or a parameter, cannot contain a catch-all parameter.";
                    return false;
                }
            }
                ...
                if (part is RoutePatternParameterPart parameter && parameter.IsOptional && parts.Count > 1)
                {
                    ...
                    if (i == parts.Count - 1)
                    {
                        ...
                        if (!previousPart.IsLiteral && !previousPart.IsSeparator)
                        {
                            ...
                            context.Error = string.Format("In the segment '{0}', the optional parameter '{1}' is preceded by an invalid segment '{2}'. Only a period (.) can precede an optional parameter.",
                    ...
                    else
                    {
                        ...
                        context.Error = string.Format("An optional parameter must be at the end of the segment. In the segment '{0}', optional parameter '{1}' is followed by '{2}'.",
                            RoutePatternPathSegment.DebuggerToString(parts),
                            parameter.Name,
                            parts[i + 1].DebuggerToString());
            ...
            for (var i = 0; i < parts.Count; i++)
            {
                ...
                {
                    context.Error = "A path segment cannot contain two consecutive parameters. They must be separated by a '/' or by a literal string.";
                    return false;
                }
        ...
        private static bool IsValidParameterName(Context context, string parameterName)
        {
            ...
            {
                context.Error = string.Format("The route parameter name '{0}' is invalid. Route parameter names must be non-empty and cannot contain these characters: '{{', '}}', '/'. The '?' character marks a parameter as optional, and can occur only at the end of the parameter. The '*' character marks a parameter as catch-all, and can occur only at the start of the parameter.", parameterName);
                return false;
            }

            if (!context.ParameterNames.Add(parameterName))
            {
                context.Error = string.Format("The route parameter name '{0}' appears more than one time in the route template.", parameterName);
                return false;
            }
