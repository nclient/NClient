...
namespace NClient.Core.AspNetRouting
{
    ...
    internal static class RoutePatternFactory
    {
            ...
            IEnumerable<RoutePatternPathSegment> segments)
        {
            ...
            if (parameterPolicies != null && parameterPolicies.Count > 0)
            {
                ...
                foreach (var kvp in parameterPolicies)
                {
                    ...
                    {
                        throw new InvalidOperationException(string.Format("Invalid constraint '{0}'. A constraint must be of type 'string' or '{1}'",
                            kvp.Value ?? "null",
            ...
            RoutePatternPart VisitPart(RoutePatternPart part)
            {
                ...
                if (updatedDefaults != null && updatedDefaults.TryGetValue(parameter.Name, out var newDefault))
                {
                    ...
                    {
                        var message = string.Format("The route parameter '{0}' has both an inline default value and an explicit default value specified. A route parameter cannot contain an inline default value when a default value is specified explicitly. Consider removing one of them.", parameter.Name);
        ...
        public static RoutePatternLiteralPart LiteralPart(string content)
        {
            ...
            {
                throw new ArgumentException(string.Format("The literal section '{0}' is invalid. Literal sections cannot contain the '?' character.", content));
            }
        ...
        public static RoutePatternParameterPart ParameterPart(string parameterName)
        {
            ...
            {
                throw new ArgumentException(string.Format("The route parameter name '{0}' is invalid. Route parameter names must be non-empty and cannot contain these characters: '{{', '}}', '/'. The '?' character marks a parameter as optional, and can occur only at the end of the parameter. The '*' character marks a parameter as catch-all, and can occur only at the start of the parameter.", parameterName));
            }
            ...
            RoutePatternParameterKind parameterKind)
        {
            ...
            {
                throw new ArgumentNullException("An optional parameter cannot have default value.", nameof(parameterKind));
            }
        ...
        public static RoutePatternParameterPolicyReference Constraint(string constraint)
        {
            ...
            {
                throw new ArgumentException("Value cannot be null or empty.", nameof(constraint));
            }
        ...
        public static RoutePatternParameterPolicyReference ParameterPolicy(string parameterPolicy)
        {
            ...
            {
                throw new ArgumentException("Value cannot be null or empty.", nameof(parameterPolicy));
            }
