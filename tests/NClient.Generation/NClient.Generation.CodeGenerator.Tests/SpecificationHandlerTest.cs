...
namespace NClient.Generation.CodeGenerator.Tests
{
  [Parallelizable]
  public class SpecificationHandlerTest
  {
    ...
    {
      var resource = typeof(SpecificationHandlerTest).GetTypeInfo().Assembly.GetManifestResourceStream($"{nameof(NClient)}.{nameof(Generation)}.{nameof(CodeGenerator)}.{nameof(Tests)}.Specifications.{name}") ?? throw new NullReferenceException("no open api spec");
    ...
    private string GenerateSourceCode(string openApiSpec)
    {
      ...
      var result = specificationHandler.Generate(openApiSpec, @namespace).GetAwaiter().GetResult();
    ...
    private OpenApiDocument GetOpenApiDoc(string openApiSpec)
    {
